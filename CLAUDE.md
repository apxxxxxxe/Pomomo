...
### テスト方針の特徴
1. **網羅的なテストケース**: 各機能に対してエッジケースを含む正常系・異常系の両方をカバー
2. **Discord APIをモック化**: 実際のDiscord APIを呼び出さずにテスト実行
3. **非同期処理のサポート**: async/awaitを使ったコードのテストに対応
4. **環境分離**: `TESTING=1`環境変数でテスト環境を識別
5. **フィクスチャベース**: pytest fixtureを活用した再利用可能なテストセットアップ
6. **モジュール独立性**: 各Cogや機能ごとに独立したテストファイル
...
- テスト実行時にはタイムアウトを設定し、無限ループなど不適切な動作が発生していないかを確認する

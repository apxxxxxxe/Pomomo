...
COMMANDS = {'コントロールコマンド': {'start': ['start [pomodoro] [short_break] [long_break] [intervals]',
            ...
            'サブスクリプションコマンド': {'dm': ['dm', 'サーバーのセッションのDMアラート購読を切り替え'],
                                      'autoshush': ['autoshush [all]', 'ポモドーロインターバル中に自動的に'
                                                                       'スピーカーとマイクをオフにする購読を切り替え。\n'
                                                                       'ミュートとスピーカーオフの権限を持つメンバーは'
                                                                       '"all"パラメータを追加してポモドーロ音声チャンネルの'
                                                                       '全員をauto_shushできます。']}}

LINKS = 'Pomomoをあなたのサーバーに招待するには[こちら]' \
        '(https://discord.com/api/oauth2/authorize?client_id=821952460909445130&permissions=15738880&scope=bot)！\n' \
        '提案やバグ報告がある場合は、サポートサーバー[こちら]' \
        '(https://discord.gg/Aghy78wcFr)を訪問するか、[top.gg](https://top.gg/bot/821952460909445130)の' \
        'ページで詳細をご確認ください。\n\n' \
        'コーヒーを買って[こちら](https://www.buymeacoffee.com/benjamonn)でPomomoをサポートすることもできます :)'
